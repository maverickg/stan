...
The \code{stan::agrad} library was developed to calculate gradients of
...
\refsection{vectorization} for the vectorized implementation of
\code{normal\_log}).
%
\begin{quote}
\begin{Verbatim}
...
\code{chain()} requiring two addition operations each, for a total of
...
\end{quote}
%
Although this class is used widely through \code{stan::agrad}, for
simplicity, the only constructor shown is for a standard vector of
autodiff variables, \code{vector<var>}.  

The \code{var\_sum} function sums the values of the variables in the
vector.  
%
\begin{quote}
\begin{Verbatim}
...
\end{quote}
%
The \code{alloc()} method does the allocation of the bytes in the
...
the intermediate values are pushed onto a \code{vector<var>} and then
the \code{sum()} function is used to return their value.  The linear
