...
y          for (int j = 0; j < x.size(); ++j)
    ...
    // //       for (int k = 0; k < x.size(); ++k)
    // //         x_fvar(k) = fvar<fvar<T> >(fvar<T>(x(k),j==k),  /** BREAKS **/
    // //                                    fvar<T>(i==k,0));
    ...
    // // **** WARNING **** 
    // // only works for double as is
    // // need to specialize for var and fvar<T> to get double value
    // // template metaprogram to pull out value generalized to fvar
    ...
    // // *** WARNING ***
    // // same restrictions as hessian() for now
    // // can be generalized to different types for x and v
    // // as in gradient_dot_vector()
